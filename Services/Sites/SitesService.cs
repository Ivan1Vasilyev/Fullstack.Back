...
namespace Backend.Services.Sites
{
    ...
    {
        public async Task<Site> CreateAsync(CreateSiteRequest request)
        {
            ...
            try
            {
                return await sitesRepository.CreateAsync(providerId, domain, request.YandexCounterKey)
                    ?? throw new InfrastructureException("Сайт не был создан");
            }
            catch (Exception ex) when (ex is ConflictException or ForeignKeyException or InfrastructureException)
            {
                throw;
            }
            catch (Exception ex)
        ...
        public async Task<IEnumerable<Site>> GetByProviderIdAsync(int providerId)
        {
            ...
            try
            {
                return await sitesRepository.GetByProviderIdAsync(providerId);
            }
        ...
        public async Task<Site> UpdateAsync(UpdateSiteRequest request)
        {
            var id = request.Id;
            if (id < 1)
